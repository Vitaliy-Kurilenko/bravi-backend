...
**Характеристики товару:** набір характеристик товару визначає його категорія, і **прив'язка
успадковується вниз по дереву категорій** — «Матеріал» вішається один раз на «Одяг», його бачать усі
підкатегорії. Ефективний набір **обчислюється** при кожному читанні, а не зберігається, тому зміна
батьківської категорії перераховує його без міграції даних. Характеристика зі `scope = GLOBAL`
доступна кожному товару магазину без жодної прив'язки, зокрема товару без категорії.
Типи значень: `TEXT`, `NUMBER` (з одиницею виміру з `dictionaries`), `BOOLEAN`, `DATE`, `SELECT`,
`MULTI_SELECT`. Обов'язковості (`required`) у цій ітерації немає.

Щоб не заводити «Колір» вручну в кожному магазині, є **глобальна бібліотека шаблонів**
(`attribute_templates`, наповнюється Flyway-seed'ом): `GET /sellers/attribute-templates` показує
її та позначає вже підключені. Підключення відбувається одним викликом
`POST /sellers/categories/{publicId}/attributes` з `template_codes` — шаблон копіюється в
`store_attributes` магазину разом з опціями (зберігаючи `template_code` як стабільний ключ для
майбутнього мапінгу на маркетплейси) і одразу прив'язується до категорії. Далі продавець вільно
редагує назву й додає власні опції.

Ендпоінти (усі — `X-Store-Id`, права `PRODUCT_READ` / `PRODUCT_WRITE`):
`/sellers/attribute-templates` (бібліотека); `/sellers/attributes` (CRUD визначень, `.../options`
для списку значень, `.../value-suggestions` — раніше введені значення TEXT-характеристик);
`/sellers/categories/{publicId}/attributes` (ефективний набір із позначкою `source` —
`GLOBAL`/`INHERITED`/`OWN` — та прив'язка/відв'язка); `/sellers/products/{publicId}/attributes`
(`GET` + `PUT` для заміни всього набору, `POST .../copy-from` — копіювання з іншого товару);
`POST /sellers/product-attributes/bulk` — однакові значення для багатьох товарів одразу.
`POST`/`PATCH /sellers/products` теж приймають масив `attributes`, тож товар із характеристиками
заводиться одним запитом.

Семантика видалення: характеристику чи опцію, які вже мають значення в товарах, видалити не можна
(409). Відв'язка характеристики від категорії **не видаляє** значення товарів — вони й далі
повертаються з `"offered": false`, щоб їх було видно, а не втрачено мовчки. `code` характеристики
унікальний у межах магазину й незмінний після створення (перейменовується `name`).

**Знижки на товар:** знижка — не поле «стара ціна», а окремий запис із періодом дії, тож у товару їх
кілька й вони вишиковуються в чергу. Статус (`SCHEDULED` / `ACTIVE` / `EXPIRED`) **не вводиться
руками** — він обчислюється з періоду й поточного часу, тому переходи відбуваються самі, без крону
й фонових задач. Тип — `PERCENT` (0.01–99) або `AMOUNT` (менша за ціну товару); `ends_at: null`
означає безстрокову знижку, яка діє, доки її не приберуть. `label` — внутрішня назва акції для
продавця, покупцеві вона не показується.

Головний інваріант: **у будь-який момент часу товар має не більше однієї чинної знижки**. Періоди
одного товару не перетинаються (межа напіввідкрита, тож `[1–10)` і `[10–20)` — це не перетин), а
безстрокова знижка займає всю вісь після старту й не може передувати запланованій. Інваріант
тримають два шари: доменна перевірка, яка віддає 409 із назвою конфліктної знижки, і
exclusion-констрейнт PostgreSQL, який закриває гонку двох менеджерів. **Констрейнт вимагає
розширення `btree_gist`** — міграція `V24` створює його сама (`CREATE EXTENSION IF NOT EXISTS`),
але роль Flyway має бути власником БД; на керованому інстансі з обмеженими правами розширення
створює DBA заздалегідь, і тоді міграція стає no-op.

Кінцеву ціну рахує **бекенд** і віддає разом із товаром, щоб адмінка, список товарів і майбутня
вітрина не розійшлися в округленні: `GET /sellers/products` і `GET /sellers/products/{publicId}`
несуть `discounted_price` та `active_discount` (`public_id`, `type`, `value`, `label`, `ends_at`);
обидва `null`, коли жодна знижка не діє. Округлення — до копійок (`HALF_UP`, два знаки), причому
округлюється сума економії, тож `discounted_price + економія` завжди дорівнює ціні точно. Ціна ніде
не зберігається — вона обчислюється при кожному читанні від актуальної ціни товару.

Ендпоінти (`X-Store-Id`, права `PRODUCT_READ` / `PRODUCT_WRITE`):
`GET /sellers/products/{publicId}/discounts` — розклад товару;
`PUT /sellers/products/{publicId}/discounts` — заміна всього набору однією транзакцією (запис із
`public_id` лишається і зберігає `created_at`, запис без нього створюється, а пропущений —
видаляється; це і є спосіб зупинити чинну знижку достроково). Похідні поля відповіді (`status`,
`created_at`, `updated_at`) у запиті ігноруються, тож прочитаний розклад можна надіслати назад
як є. `POST /sellers/product-discounts/bulk` — одна знижка на список товарів; товар, у якого період
уже зайнятий або чия ціна не витримує фіксовану суму, потрапляє в `skipped` із причиною, замість
завалити весь запит. У списку товарів працює фільтр `has_active_discount=true|false`.

Що змінити не можна: у **чинної** знижки заблокована дата початку (решту полів редагувати можна) —
зупинити її достроково можна лише видаленням. Зміна ціни товару перевіряється проти живих
`AMOUNT`-знижок: `PATCH /sellers/products/{publicId}` з ціною, яка не перевищує таку знижку,
відхиляється з 400 на полі `price`. Замовлення фіксують ціну на момент оформлення — зміна чи
видалення знижки історію не переписує.

**Теги:** тег — це мітка з довідника магазину, а не вільний текст на сутності, тож перейменування
робиться в одному місці, а фільтр працює по стабільних id. Тег **суто внутрішній** — його бачить
лише продавець, як `label` у знижки, — тому в ньому немає нічого, крім назви, кольору й статусу:
ні slug'а, ні опису, ні порядку сортування.

Довідник **один на всі сутності**, які можна тегувати, а розрізняє їх колонка `target`
(`products` зараз, `orders` — наступна ітерація). Словники роздільні: «Хіт» у товарів і «Терміново»
в замовлень не бачать одне одного, тому `target` входить у ключ унікальності
`(store_id, target, lower(name))`. Зв'язки натомість мають окрему таблицю на кожну сутність —
поліморфний FK неможливий, а саме `ON DELETE CASCADE` прибирає зв'язки разом із товаром.

Теги **створюються на льоту**: назва, якої магазин ще не має, стає тегом просто тому, що її вписали
в картці товару. Тому справжній ключ довідника — це нормалізована назва, а не `public_id`: назва
обрізається по краях, внутрішні пробіли згортаються в один, і збіг шукається без урахування
регістру, тож `«  Хіт  »`, `«хіт»` і `«ХІТ»` — це один тег (написання зберігається те, що ввели
першим). Дедуплікацію тримає унікальний індекс, а не перевірка «прочитати й записати», тому два
паралельні збереження товару з однією новою назвою не створять дубль і не завалять одне одного.
Максимальна довжина назви — 64 символи.

**Колір тега.** Колір задається в форматі `#RRGGBB` і зберігається рівно в такому вигляді:
значення обрізається по краях, переводиться у верхній регістр, а скорочена форма розгортається
(`#f80` → `#FF8800`). Усе, що не є hex-кольором, відхиляється з 400 на полі `color`; ту саму форму
дублює констрейнт `ck_store_tags_color`, тож БД і застосунок описують одне й те саме.

Колір є **завжди** — стану «без кольору» не існує. Якщо його не вказали (зокрема коли тег
створюється на льоту по назві в картці товару), він береться з фіксованої палітри, причому
детерміновано за назвою: та сама назва завжди дає той самий колір, тож бейдж не «стрибає» між
запитами й не потребує ані лічильника, ані додаткового запиту. `PATCH` без поля `color` колір не
чіпає; скинути його назад в автоматичний не можна — можна лише задати інший.

Ендпоінти довідника універсальні, `{target}` — це `products` або `orders`:
`GET|POST /sellers/tags/{target}`, `GET|PATCH|DELETE /sellers/tags/{target}/{publicId}`,
`POST /sellers/tags/{target}/{publicId}/merge`. Право визначається типом тега (`PRODUCT_*` для
товарних, `ORDER_*` — для тегів замовлень), нових кодів прав не додається. Призначення тегів
лишається окремим на кожну сутність, бо приналежність сутності магазину доводить її власний модуль:
`GET|PUT /sellers/products/{publicId}/tags` і `POST /sellers/product-tags/bulk` (`mode`:
`ADD` / `REMOVE` / `REPLACE`, за замовчуванням `ADD`; у відповіді — скільки товарів реально
змінилось). У `REMOVE` назви нічого не створюють: невідома назва просто ні з чим не збігається.

Теги можна передати одразу в тілі товару (`tags` у `POST`/`PATCH /sellers/products`): запис — це
`{"id": "..."}` для наявного тега або `{"name": "..."}` для збігу чи створення. На `PATCH` діє те
саме правило, що й для характеристик: відсутнє поле лишає набір як є, `[]` — очищає. У списку та
картці товару теги повертаються як `tags: [{"id", "name", "color"}]` — на відміну від характеристик,
вони є і на сторінці списку, бо саме там їх читають. Це окремий тип, а не спільний `CatalogRefResponse`
категорії й виробника: колір має сенс лише для тега.

Фільтр у пошуку товарів: `?tag_ids=a,b&tags_match=any|all` (за замовчуванням `any` — досить одного
збігу; `all` вимагає всі). Невідомий id у фільтрі — 404, щоб застаріле посилання не поверталося
мовчки з усім каталогом.

Видалення тега **успішне** й просто знімає його з усіх товарів (на відміну від характеристики, яку
видалити не дадуть, поки вона використовується): характеристика несе типізовані дані, а тег — лише
мітку. Скільки товарів це зачепить, видно заздалегідь у полі `usage_count`. Перейменування в уже
зайняту назву — 409 із підказкою про `merge`, а не тихе злиття; `merge` переносить зв'язки на
цільовий тег (товар, що ніс обидва, лишається з одним) і видаляє джерела.
