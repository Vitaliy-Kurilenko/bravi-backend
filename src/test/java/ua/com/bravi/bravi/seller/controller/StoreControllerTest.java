...
class StoreControllerTest {
    ...
    private final StoreContext storeContext = mock(StoreContext.class);
    private final SellerAccountResolver sellerAccountResolver = mock(SellerAccountResolver.class);
    private final SellerStoresController controller =
            new SellerStoresController(storesApi, storeDtoMapper, storeLogoDtoMapper, storeContext, sellerAccountResolver);

    @Test
    void listStoresReturnsAccountStores() {
        StoreView view = mock(StoreView.class);
        StoreResponse response = mock(StoreResponse.class);
        when(sellerAccountResolver.resolveSellerAccountId()).thenReturn(7L);
        when(storesApi.getStoresByAccountId(7L)).thenReturn(List.of(view));
        when(storeDtoMapper.toResponse(view)).thenReturn(response);

        List<StoreResponse> result = controller.listStores();
    ...
    void getStoreReturnsMappedResponse() {
        StoreView view = mock(StoreView.class);
        StoreResponse response = mock(StoreResponse.class);
        when(storeContext.get()).thenReturn(42L);
        when(storesApi.getStoreById(42L)).thenReturn(Optional.of(view));
        when(storeDtoMapper.toResponse(view)).thenReturn(response);

        StoreResponse result = controller.getStore("st_1");
    ...
    void updateStoreReturns204AndDelegatesToApi() {
        ...
        ResponseEntity<Void> result = controller.updateStore("st_1", request);
    ...
    void updateStoreAttachesLogoWhenStorageKeyPresent() {
        ...
        controller.updateStore("st_1", request);
